...
# A. MOVE THE FRESH FOLDER FIRST (Ignore time, just find the name)
NEW_FOLDER=$(find . -maxdepth 1 -type d -name "*_adhoc_*" | head -n 1)

if [ -n "$NEW_FOLDER" ]; then
    echo "Organizing $NEW_FOLDER into LatestBackup..."
    rm -rf "LatestBackup"
    mv -f "$NEW_FOLDER" "LatestBackup"
    echo "Success: LatestBackup updated."
else
    echo "No new adhoc folder found. Checking if LatestBackup is already current..."
fi

# B. CLEANUP (Delete any OTHER adhoc folders older than 1 day)
...
if [ "$(date +%u)" -eq 7 ]; then
    ...
    if docker exec $CONTAINER restic \
       ...
       backup "/data/LatestBackup" --host truenas-home && \
...
06:05	Cloud Sync (Sun)	Restic	Uploads the static /LatestBackup/ path to the Cloud.
